...
def _capture_create_env(env_config, task_id="t1"):
    ...
    assert env_type == env_config["env_type"]
    return captured
...
def test_execute_code_uses_canonical_terminal_container_projection():
    ...
    assert actual == _container_config_from_config(cfg)
...
def test_plugin_backend_receives_container_policy(is_container):
    """A registered sandbox gets the same policy as built-in containers."""
    from agent import terminal_env_registry
    from agent.terminal_env_provider import TerminalEnvironmentProvider
    from tools.terminal_tool import _container_config_from_config

    class PolicyBackend(TerminalEnvironmentProvider):
        name = "fork-policy-test"

        def is_available(self):
            return True

        def create_environment(self, **kwargs):
            raise AssertionError("factory is intercepted by the harness")

    backend = PolicyBackend()
    backend.is_container = is_container
    previous = terminal_env_registry.get_provider(backend.name)
    terminal_env_registry.register_provider(backend)
    try:
        config = _make_env_config(env_type=backend.name)
        actual = _capture_create_env(config)["container_config"]
        expected = _container_config_from_config(config) if is_container else None
        assert actual == expected
    finally:
        terminal_env_registry.restore_registration(backend.name, backend, previous)
