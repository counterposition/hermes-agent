...
def _disable_real_macos_keychain(monkeypatch):
    """Keep auth tests hermetic on developer Macs with Claude Code installed."""
    monkeypatch.setattr(
...
class TestBuildAnthropicKwargs:
    ...
    def test_max_uses_highest_legacy_manual_budget(self):
        kwargs = build_anthropic_kwargs(
            model="claude-sonnet-4-20250514",
            messages=[{"role": "user", "content": "think hard"}],
            tools=None,
