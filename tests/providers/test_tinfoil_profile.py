...
class TestTinfoilReasoningRequestShape:
    ...
    def test_kimi_efforts_are_clamped(self, profile, effort, expected):
        kwargs = _build_request(profile, "kimi-k2-6", {"effort": effort})
    ...
    @pytest.mark.parametrize("model", ["deepseek-v4-pro", "glm-5-2", "gemma4-31b"])
    ...
    def test_full_range_efforts_pass_through(self, profile, effort):
        kwargs = _build_request(profile, "deepseek-v4-pro", {"effort": effort})
    ...
    @pytest.mark.parametrize("model", ["kimi-k2-6", "gpt-oss-120b"])
    ...
    @pytest.mark.parametrize(
        ...
        [("deepseek-v4-pro", None), ("future-secure-model", {"effort": "high"})],
    )
...
def test_always_reasoning_models_use_supported_efforts(profile, model, effort):
    request = _build_request(profile, model, {"effort": effort})
...
def test_glm53_does_not_send_unsupported_disable(profile, model):
    request = _build_request(profile, model, {"enabled": False})
