...
class TestQwenReplay:
    def test_qwen_replay_preserves_native_reasoning_without_mutating_history(self, profile):
        from copy import deepcopy
        from types import SimpleNamespace

        source = {"role": "assistant", "content": "answer", "reasoning": "thought"}
        original = deepcopy(source)
        wire = dict(source)
        copy_reasoning_content_for_api(
            SimpleNamespace(provider="cerebras", model="qwen-3.8-27b"), source, wire
        )
        request = TestCerebrasTransportIntegration()._build(
            "qwen-3.8-27b", [wire], {"reasoning_config": {"effort": "ultra"}}
        )
        assert request["messages"][0]["reasoning"] == "thought"
        assert request["messages"][0]["content"] == "answer"
        assert "reasoning_content" not in request["messages"][0]
        assert request["reasoning_effort"] == "high"
        assert source == original

    def test_qwen_disable_is_explicit(self, profile):
        assert profile.build_api_kwargs_extras(
            model="qwen-3.8-27b", reasoning_config={"enabled": False}
