...
class TestCerebrasProfile:
    ...
    def test_api_key_auth_and_env_vars(self):
        ...
        assert p.env_vars == ("CEREBRAS_API_KEY",)
    ...
    def test_fallback_models_curated(self):
        ...
        assert list(p.fallback_models) == _PROVIDER_MODELS[p.name]
    ...
    def test_reasoning_effort_for_reasoning_models(self):
        ...
        # Cerebras documents only ``none`` for GLM; enabled requests omit the
        # field rather than sending GPT-OSS's low/medium/high values.
        assert top == {}
