...
class CerebrasProfile(ProviderProfile):
    ...
    def project_assistant_replay(
        ...
        *,
        ...
        """Project reasoning using the model family's documented wire format."""
        reasoning = self._reasoning_text(source_msg)
        source_content = source_msg.get("content")
        wire_content = api_msg.get("content")

        # Current Qwen endpoints accept historical reasoning as a separate
        # `reasoning` field. Keep it separate from the visible answer.
        if "qwen-3.8" in (model or "").lower():
            api_msg.pop("reasoning_content", None)
            api_msg.pop("reasoning", None)
            if reasoning:
                api_msg["reasoning"] = reasoning
            return True
    ...
    def prepare_messages_for_model(
        ...
        *,
        ...
        # while current Qwen accepts the separate ``reasoning`` field. This
        # is a defense-in-depth path for
    ...
    def build_api_kwargs_extras(
        ...
        **context: Any,
        ...
        # GPT-OSS and Qwen accept low/medium/high reasoning_effort. Legacy GLM documents
        # ``none`` as an accepted value (to disable reasoning); enabled GLM
        # requests must omit the field. Qwen additionally accepts none. Cerebras is
        ...
        ml = (model or "").lower()
        is_glm = "glm" in ml
        is_qwen = "qwen-3.8" in ml
        if "gpt-oss" not in ml and not is_glm and not is_qwen:
            return {}, {}
        ...
        if isinstance(reasoning_config, dict) and reasoning_config.get("enabled") is False:
            return ({}, {"reasoning_effort": "none"}) if is_glm or is_qwen else ({}, {})
        if is_glm:
            return {}, {}
